...
<img src="assets/cute_backbone.png" alt="CuteTTS 模型架构" width="100%">
...
`generate_stream()` 会在音频块解码完成后立即返回位于 CPU 上的 `float32` PCM 数据：
...
| <sub>模型</sub> | <sub>参数量</sub> | <sub>LibriSpeech test-clean WER (%) ↓</sub> | <sub>LibriSpeech test-clean SIM ↑</sub> | <sub>Seed-TTS EN WER (%) ↓</sub> | <sub>Seed-TTS EN SIM ↑</sub> | <sub>Seed-TTS ZH WER (%) ↓</sub> | <sub>Seed-TTS ZH SIM ↑</sub> |
|:--|--:|--:|--:|--:|--:|--:|--:|
...
- [Descript Audio Codec（DAC）](https://github.com/descriptinc/descript-audio-codec)：Audio VAE 部分实现
- [F5-TTS](https://github.com/SWivid/F5-TTS)：Sway Sampling 调度
- [Qwen3](https://github.com/QwenLM/Qwen3)：基于[Hugging Face Transformers v4.51.0](https://github.com/huggingface/transformers/tree/v4.51.0/src/transformers/models/qwen3)的模型结构

## 许可证

CuteTTS 使用 Apache License 2.0 发布。第三方代码部分继续保留各自的版权和许可声明，详见 [NOTICE](NOTICE)。
